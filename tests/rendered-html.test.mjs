...
test("portfolio card includes unit accounting, reset, prices and daily NAV", async () => {
  ...
  assert.ok(data.holdings.every((holding) => Number.isFinite(holding.risk.sharpe)));
  assert.ok(data.holdings.every((holding) => Number.isFinite(holding.risk.volatility)));
  assert.ok(data.holdings.every((holding) => Number.isFinite(holding.risk.skewness)));
  assert.ok(data.holdings.every((holding) => Number.isFinite(holding.risk.excessKurtosis)));
  assert.ok(data.holdings.every((holding) => Number.isFinite(holding.risk.maxDrawdown)));
  assert.ok(Number.isFinite(data.riskSummary.sharpe));
  assert.ok(Number.isFinite(data.riskSummary.weightedStockSharpe));
  assert.ok(data.riskSummary.observations >= 200);
