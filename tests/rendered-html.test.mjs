...
test("portfolio card includes interactive period returns, chart, positions and portfolio distribution", async () => {
  ...
  assert.match(page, /<span>\{active\.point\.date\}<\/span><strong>\{signedPct\(active\.value\)\}/);
  assert.doesNotMatch(page, /signedPct\(active\.point\.dailyReturn\)/);
  ...
  assert.ok(data.summary.activeCashWeight >= 0 && data.summary.activeCashWeight <= 1);
