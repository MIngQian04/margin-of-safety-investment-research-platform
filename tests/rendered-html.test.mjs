...
test("portfolio card includes interactive period returns, chart, positions and portfolio distribution", async () => {
  ...
  assert.equal(data.activeAsOf, "2026-07-17");
  assert.equal(data.distributionAsOf, data.activeAsOf);
  assert.equal(data.summary.activeCashWeight, 0.3250000000000002);
  ...
  assert.equal(data.allocationChange.changes[0].code, "300628.SZ");
  assert.equal(data.allocationChange.changes[0].newWeight, 0.125);
  assert.match(data.allocationChange.changes[0].reason, /乐观DCF情景/);
  ...
  assert.equal(data.navHistory.at(-1).dailyReturn, 0.0232752358327728);
});
