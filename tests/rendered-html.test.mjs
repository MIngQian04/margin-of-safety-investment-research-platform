...
test("portfolio card includes interactive period returns, chart, positions and portfolio distribution", async () => {
  ...
  assert.equal(data.allocationChange.changes.length, 2);
  assert.equal(data.allocationChange.changes[0].code, "000786.SZ");
  assert.equal(data.allocationChange.changes[0].newWeight, 0.1);
  assert.match(data.allocationChange.changes[0].reason, /房地产下行暴露尚未充分缓解/);
  assert.equal(data.allocationChange.changes[1].code, "600941.SH");
  assert.equal(data.allocationChange.changes[1].newWeight, 0.075);
  assert.match(data.allocationChange.changes[1].reason, /牌照、频谱、全国网络/);
  ...
  assert.ok(data.nextHoldings.some((holding) => holding.code === "600941.SH" && holding.weight === 0.075));
