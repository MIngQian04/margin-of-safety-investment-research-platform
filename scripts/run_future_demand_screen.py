...
- 五档DCF采用历史三年所有者收益中位数，只负责提供现有业务价值底线和失败、部分兑现、成功三种现金流情景，不能把尚未验证的新业务直接当成既成利润。
- 最终种子仓准入在组合阶段另做里程碑概率加权：成功概率只能随带日期、来源的需求、利润池和公司兑现证据提高；概率加权安全边际至少30%，且失败情景下行不超过30%。本页的初筛状态不是最终建仓结论。
- 已持有种子仓仍采用预警和逐档减仓，模型升级不会一次性改写已经发布的历史仓位；没有新证据时冻结加仓和晋级。
